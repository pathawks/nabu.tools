...
export class PowerSave3DSDriver implements NDSDeviceDriver {
  ...
  async detectCartridge(
    ...
    if (!this.header || this.headerChipId !== chipIdHex) {
      this.log(`Chip ID: ${Array.from(id, hex).join(" ")}`);
  ...
  private async probeSaveChip(): Promise<{
    ...
    if (!tinyBase.every((b) => b === tinyBase[0])) {
      ...
      if (eepromWrapsAt(tinyBase, wrap)) {
        return { size: 0x100, typeName: "EEPROM", addrWidth: 1 };
      }
    }
...
/**
 * Test whether a 16-byte read at the wrap boundary of an `sz`-byte SPI
 * EEPROM matches the same chip's first 16 bytes shifted by one — i.e.
 * `wrapRead[1..15] === base[0..14]`. byte 0 of `wrapRead` is the byte
 * at the wrap address itself, which can be anything.
 */
export function eepromWrapsAt(
  base: Uint8Array,
  wrapRead: Uint8Array,
): boolean {
  for (let i = 0; i < 15; i++) {
    if (wrapRead[i + 1] !== base[i]) return false;
  }
  return true;
}
